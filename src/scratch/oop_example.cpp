...
int main(int argc, char** argv)
{
	...
	Person* pptr;						// Declaring a pointer to a Person (NOT A PERSON!!!)
										//	  uninialized at the moment;
	pptr = NULL;				// Null is a macro (the value 0)
	int x = NULL;				// Assigning the VALUE of 0 to x (x is NOT a pointer)
	...
	std::cout << "Doing some more stuff with the Person pointed to by the pptr\n";
	...
	delete pptr;							// Freeing the memory pointed to by pptr. pptr's still
											...
											//   use that address or bad things will happen.
	...
	delete pptr;							// This calles the Destructor, then frees up this chunk
